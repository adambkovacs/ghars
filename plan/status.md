...
- Search now uses pulled README context in addition to repo facts and notes
- Repo detail now pulls and caches upstream README content for imported repos
- Analytics now consumes imported portfolio data, stored repo snapshots, coverage metrics, action queues, and portfolio activity
...
| 3. Dashboard alpha and chart system | partial | Dashboard and analytics are live on imported data with stored snapshot trends, meaningful coverage/action queues, and activity feeds, but drill-down depth is still incomplete | [dashboard page](../app/dashboard/page.tsx), [analytics page](../app/analytics/page.tsx), [sparkline](../components/charts/sparkline.tsx) |
...
| 5. AI assist and cluster intelligence | partial | README enrichment is now live and persisted, but AI-generated summaries and richer cluster intelligence are still incomplete | [README gateway](../lib/adapters/github/githubApiGateway.ts), [README runtime helper](../lib/server/portfolio/readme.ts), [cluster narratives](../lib/services/buildClusterNarratives.ts), [report generation](../lib/services/generateReports.ts), [service tests](../tests/unit/services.test.ts) |
...
| Search | yes | yes with README-aware recall | yes through post-import search flow | done |
| Analytics | yes | yes with stored snapshot history, coverage, activity, and action queues | yes through post-import analytics flow | partial |
| Reports | yes | yes with persisted snapshots | yes through post-import reports flow | partial |
| Repo detail | yes | yes with live note/state mutation and cached README content | yes through search drill-down flow | done |
...
- README enrichment is now persisted and reused across import, repo detail, search, scheduled refresh, and reporting: [README helper](../lib/server/portfolio/readme.ts), [GitHub adapter](../lib/adapters/github/githubApiGateway.ts), [Convex runtime and stores](../lib/server/portfolio/runtime.ts), [Convex refresh action](../convex/github.ts), [Convex schema](../convex/schema.ts)
...
- Analytics and reports now read from the shared imported-portfolio runtime instead of demo data, with snapshots and reports persisted through the shared artifact builder and analytics exposing coverage, opportunity, neglect, and activity sections: [analytics page](../app/analytics/page.tsx), [reports page](../app/reports/page.tsx), [artifact builder](../lib/server/portfolio/artifacts.ts), [runtime assembly](../lib/server/portfolio/runtime.ts), [Playwright flow](../e2e/smoke.spec.ts)
